...
export default function UploadFiles() {
  ...
  const handleFileChange = (details: any) => {
    console.log(details);
    if (details && details.value && Array.isArray(details.value)) {
      setFiles(details.value);
    }
    setUploadError(null);
  };
  ...
  const handleFileReject = (rejectedFiles: { file: File; errors: any[] }[]) => {
    if (rejectedFiles.length > 0) {
      setUploadError(
        `${rejectedFiles.length} file(s) couldn't be added: invalid file type or size limit exceeded`
      );
    }
  };
          ...
          <Show
            ...
            fallback={
              <FileField
                multiple
                allowDragAndDrop
                maxFileSize={15 * 1024 * 1024} // 15MB max per file
              ...
              >
                ...
                <FileField.Dropzone class="border-2 border-dashed rounded-lg p-6 mb-4 text-center transition-colors hover:border-primary hover:bg-primary-lighter data-[valid-drop-target]:border-primary data-[valid-drop-target]:bg-primary-lighter">
                ...
                <Show when={true}>
                  <div class="mt-4">
                    <FileField.Description class="text-sm text-neutral-light mb-2">
                      {files().length} file(s) selected. Click "Upload" to start
                      the upload process.
                    ...
                    <div class="flex justify-end gap-2">
                      ...
                      <button
                        ...
                        disabled={files().length === 0 || ctx.isUploading()}
                      >
                        Upload
                      </button>
                    </div>
